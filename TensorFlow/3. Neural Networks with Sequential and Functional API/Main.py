...
x_train = x_train.reshape(-1, 28 * 28).astype(
    "float32") / 255.0  # transformam inputurile intr un numpy vector de 784 spatii
...
model.add(layers.Dense(10, activation="softmax",
                       name="output_layer"))  # daca nu avem activare aici trebuie ca la model.compile la loss...from_ligits sa fie true
...
model.compile(
    ...
    # from logits false este pentru ca noi avem outputul nenormalizat
...
print(
    model.summary())  # putem sa printam numai dupa model.fit sau daca folosim adaugarea layerelor cu add atunci se poate si mai sus
